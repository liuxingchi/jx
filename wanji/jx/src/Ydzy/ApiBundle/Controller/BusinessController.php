...
class BusinessController extends Controller
{
    ...
    public function countAction(Request $request)
    {
		...
		if($machine_id==0){
			...
			$array_json = BusinessController::one($row['machine_id'],$start_search_date,$end_search_date);
			//echo $array_json['money']."<br>";
			$money_all = $money_all + $array_json['money'];
			$machine_num_all = $machine_num_all + 1;
			$date = $date + $array_json['date'];
			}			
			$json_result = array('money'=>$money_all,'machine_num'=>$machine_num_all,'date'=>$date,'ave_date'=>$date/$machine_num_all,'ave_money'=>($money_all/$date));
			return new JsonResponse($json_result);
		}
			
		
		else{	
				return new JsonResponse(BusinessController::one($machine_id,$start_search_date,$end_search_date));
			}	
		}
	  
    
	
	public function one($machine_id,$start_search_date,$end_search_date)
    {
