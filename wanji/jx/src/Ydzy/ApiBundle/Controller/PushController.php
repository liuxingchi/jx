...
class PushController extends Controller
{
    ...
    public function picAction(Request $request)
    {
		...
		$sql = "select b.original_image from Local_pic as a left join images as b on a.image_id=b.id limit 1";
		$pic_url = mysql_result(mysql_query($sql),0);
		$json_result=array('url'=>"http://wanjiwang.cn".$pic_url);
	...
	public function getAllAction(Request $request)
    {
		$json = $this->get('json_parser')->parse($request);
		$this->get('my_datebase')->connection();
		$sql = "select * from User_log where uid = 0 and status=1";
