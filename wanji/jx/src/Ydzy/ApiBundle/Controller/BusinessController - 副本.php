    ...
    public function countAction(Request $request)
    {
		...
		if($machine_id==0){
			...
			foreach($arraymachine as $machine){
				$machine['machine_id'];
				$sql ="select * from Business where machine_id = $machine[machine_id]";
				$result = mysql_query($sql);
		$num   = mysql_num_rows($result);
		if (!$num){
			return new Response('', 400);
		...
		foreach($arraydate as $row){
			...
			if(($row['start_date']<=$start_search_date)&&($start_search_date<=$row['end_date'])&&($row['start_date']<=$end_search_date)&&($end_search_date<=$row['end_date'])){
				...
				$json_result = array('money'=>$money,'date'=>$day_all,'ave_money'=>($money/$day_all));
		...
		$json_result = array('money'=>($money_left+$money_middle+$money_right),'date'=>($day_left+$date_middle+$day_right),'ave_money'=>(($money_left+$money_middle+$money_right)/($day_left+$date_middle+$day_right)));
		return new JsonResponse($json_result);
		}
		$array_middle_money = array();
		$array_middle_date = array();
		...
		$money_middle = array_sum($array_middle_money);
		$date_middle = array_sum($array_middle_date);
		$json_result = array('money'=>($money_middle),'date'=>($date_middle),'ave_money'=>(($money_middle)/($date_middle)));
		return new JsonResponse($json_result);
