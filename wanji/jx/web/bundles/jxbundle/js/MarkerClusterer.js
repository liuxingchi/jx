...
(function(){
    ...
    MarkerClusterer.prototype._createClusters = function(){
        ...
        for(var i = 0, marker; marker = this._markers[i]; i++){
            ...
            if(!marker.isInCluster && extendedBounds.containsPoint(marker.getPosition()) ){ 
                this._addToClosestCluster(marker,title,point);
            }
        }   
    };
     ...
     * @return 无返回值。
     */
    MarkerClusterer.prototype._addToClosestCluster = function (marker,title,point){
        ...
        if (clusterToAddTo && clusterToAddTo.isMarkerInClusterBounds(marker)){
            clusterToAddTo.addMarker(marker,title,point);
        } else {
            var cluster = new Cluster(this);
            cluster.addMarker(marker,title,point);            
     ...
     * @return 无返回值。
     */
    Cluster.prototype.addMarker = function(marker,title,point){
        ...
        if(len < this._minClusterSize ){     
			...
			//创建信息窗口
		var opts = { width: 10, height: 10}; //信息窗口 
		var infoWindow1 = new BMap.InfoWindow(title,opts);
		marker.addEventListener("click", function(){this.openInfoWindow(infoWindow1);});

			//var myCompOverlay = new ComplexCustomOverlay(point,title,title);
